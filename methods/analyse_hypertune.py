...
def hypertune_best_results(results):
    ...
    for output_steps in OUTPUT_STEPS:
        for period, end_date in PERIODS_MAP.items():
            for target_variable in TARGET_VARIABLES:
                ...
                try:
                    best_results.loc[(output_steps), (period, target_variable, 'Val')
                                     ] = output[0]['val_results']['average']
                    best_results.loc[(output_steps), (period, target_variable, 'Test')
    ...
    best_results[cols] = best_results[cols].apply(
...
def aggregate_hypertune_results(best_results):
    aggregate_results = pd.DataFrame(
        index=OUTPUT_STEPS, columns=AGGREGATE_COLUMNS)
    for output_steps in OUTPUT_STEPS:
        for target_variable in TARGET_VARIABLES:
            aggregate_results.loc[(output_steps), (target_variable, 'Val')] = np.mean(
                [best_results.loc[(output_steps), (i, target_variable, 'Val')] for i in PERIODS_MAP.keys()])
            aggregate_results.loc[(output_steps), (target_variable, 'Test')] = np.mean(
                [best_results.loc[(output_steps), (i, target_variable, 'Test')] for i in PERIODS_MAP.keys()])
...
def hypertune_best_fixed_params(results, paramater, parameter_label):
            ...
            for target_variable in TARGET_VARIABLES:
                ...
                try:
                    best_results.loc[len(best_results)] = [
                        target_variable, output_steps, period, output[0][paramater]]
                except:
                    best_results.loc[len(best_results)] = [
                        target_variable, output_steps, period, 0]

    best_results['Variable'] = best_results['Variable'].replace(
        VARIABLES_MAP)
    best_results['Output Steps'] = best_results['Output Steps'].astype(str)
    best_results[parameter_label] = pd.to_numeric(
        best_results[parameter_label]).fillna(0)
    ...
    steps_results = best_results.groupby(
        ['Output Steps'], sort=True).mean().transpose()
...
def hypertune_best_variable_params(results, paramater, parameter_label, ignore_0=False):
    ...
    for output_steps in OUTPUT_STEPS:
        for period, end_date in PERIODS_MAP.items():
            for target_variable in TARGET_VARIABLES:
                ...
                try:
                    best_results.loc[len(best_results)] = [
                        target_variable, output_steps, period, output[0]['model_parameters'][paramater]]
    ...
    if ignore_0:
        variable_results = best_results.replace(
            0, np.NaN).groupby(['Variable']).mean().transpose()

        period_results = best_results.replace(
            0, np.NaN).groupby(['Period']).mean().transpose()
        best_results['Output Steps'] = best_results['Output Steps'].astype(int)
        steps_results = best_results.replace(0, np.NaN).groupby(
            ['Output Steps'], sort=True).mean().transpose()

    else:
        variable_results = best_results.groupby(
            ['Variable']).mean().transpose()
        period_results = best_results.replace(
            0, np.NaN).groupby(['Period']).mean().transpose()
        best_results['Output Steps'] = best_results['Output Steps'].astype(int)
        steps_results = best_results.groupby(
...
def plot_3d_results(results, parameter):
    fig = px.scatter_3d(results, x='Variable', y='Output Steps', z=parameter,
                        color='Variable', width=1250, height=750)
    fig.update_layout(
        font=dict(
            size=BIGGEST_SIZE,
        )
