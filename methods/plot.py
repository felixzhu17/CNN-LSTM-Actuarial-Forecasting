...
def plot_variables(dataset, stationary=True):
    ...
    fig, axes = plt.subplots(
        nrows=number_of_variables,
        ncols=1,
        dpi=120,
        figsize=(FIG_SIZE[0], FIG_SIZE[1] * number_of_variables),
    )
...
def plot_results(
    ...
    if number_of_variables == 1:
        ...
        fig, axes = plt.subplots(
            nrows=min(len(variables), 4),
            ncols=max(int(len(variables) / 4), 1),
            dpi=120,
            figsize=(20, 10),
        )
        for i, ax in enumerate(axes.flatten()):
...
def plot_example_results(
    best_results_detailed, test_period, alphas=[0.4, 0.1], model="NN"
):

    fig, axes = plt.subplots(
        nrows=4, ncols=1, dpi=120, figsize=(FIG_SIZE[0], FIG_SIZE[1] * 4)
    )
    ...
    for i, ax in enumerate(axes.flatten()):
        ...
        target = [
            a
            for a in best_results_detailed
            if TARGET_VARIABLES[i] in a["variables"]
            and a["period"]["end"] == PERIODS_MAP[test_period]
            and a["output_steps"] == 1
        ][0]
