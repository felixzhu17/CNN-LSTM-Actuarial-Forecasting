...
class Data_Prep:
    ...
    def remove_NA_data(self, NA_cutoff: int):
        ...
        high_na = pd.DataFrame(self.raw_data.isna().sum(), columns=["NA"]).query(
            f"NA > {NA_cutoff}"
        )
        remove_variables = list(high_na.index)
        remove_variables = [i for i in remove_variables if i != "WAGE"]

        self.na_removed = self.raw_data.drop(remove_variables, axis=1, inplace=False)
    ...
    def transform_FRED_data(self):
        ...
        try:
            ...
            transformation = dict(self.data_description[["fred", "tcode"]].values)
        ...
        for i in temp_data.columns:
            ...
            try:
                ...
                print(f"Variable {i} not found in transformation dictionary")
                continue
    ...
    def transform_to_supervised_learning(
        self,
        NA_cutoff: int,
        target_variables: list,
        output_steps: int,
        ...
        for period in range(1, output_steps + 1):
            for i in target_variables:
                with_target_transformed[f"{i}(t+{period})"] = with_target_transformed[
                    f"{i}(t)"
                ].shift(periods=-1 * period)
        ...
        # Name of variables
        X_variables = [i for i in with_target_transformed.columns if i.endswith("(t)")]
        Y_variables = [
        ...
        self.supervised_dataset = Dataset(
            raw_data=with_target_raw,
            transformed_data=with_target_transformed,
