...
PERIODS = {'During COVID': 2019, 'After GFC': 2017,
           'During GFC': 2010, 'Before GFC': 2006}


class Pension_Fund():
    def __init__(self, start_date, workers=1500, retirees=500, benefits=2000, contributions=750, value=5000):
    ...
    def update_fund(self, inflation_rate, investment_returns, wage_rate, unemployment_rate):
        ...
        self.prev_value = self.prev_value * \
            (investment_returns) - self.prev_retirees * \
            self.prev_benefits + self.prev_workers*self.prev_contributions
...
def reverse_transform(results, variables_t, variables_t_1):
    ...
    for k, v in output.items():
        if k == 'GS5' or k == 'UNRATE':
            ...
            output[k] = np.exp(reverse_diff(
                np.log(variables_t[k]) - np.log(variables_t_1[k]), v))
    return output
...
def forecast_fund_value(results, period='After GFC', output_steps=24, interval=False, simulations=5000):
    ...
    if interval:
        ...
        fund_intervals = {0.4: {'lower': get_quantile(0.15), 'upper': get_quantile(0.85)},
...
def get_fund_info(results, period='After GFC', use_residual=False, actual=False):
    ...
    for variable in transformed_fund_info.keys():
        ...
        if use_residual:
            ...
            transformed_fund_info[variable] = [j[start_index]
    ...
    fund_info = reverse_transform(
        transformed_fund_info, variables_t, variables_t_1)
...
def calculate_fund_value(fund_info, output_steps, date_t):
    ...
    for i in range(output_steps):
        ...
        NN_fund.update_fund(inflation_rate, investment_returns,
                            wage_rate, unemployment_rate)
