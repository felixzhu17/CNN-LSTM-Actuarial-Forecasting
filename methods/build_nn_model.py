...
def build_model(data, model_details):
    return (
        _build_model(data, model_details)
        if "CNN_layers" in model_details["model_parameters"]
        else _build_model_no_CNN(data, model_details)
    )
...
def _build_model(data, model_details):
    ...
    for i in range(CNN_layers):
        if i == 0:
            model.add(
                Conv1D(
                    filters=model_details["model_parameters"][f"CNN_{i}_filters"],
                    kernel_size=2,
                    input_shape=(
...
def _build_model_no_CNN(data, model_details):
    ...
    for i in range(LSTM_layers):
        if i == LSTM_layers - 1:
            model.add(LSTM(model_details["model_parameters"][f"LSTM_{i}_units"]))
        else:
            model.add(
                LSTM(
                    model_details["model_parameters"][f"LSTM_{i}_units"],
    ...
    for i in range(Dense_layers):
        model.add(Dense(model_details["model_parameters"][f"Dense_{i}_units"]))
        model.add(Dropout(Dropout_prob))
