...
def get_shap_object(variable):
    ...
    NN_results = get_NN_results(model, data, VAL_STEPS, TEST_STEPS, look_back_steps, dataset,  BATCH_SIZE, EPOCHS, executions = 1)
    return NNForecastShap(NN_results.test_models[0], NN_results.data.train_X, NN_results.data.test_X, dataset['target_variables'], model_details)
