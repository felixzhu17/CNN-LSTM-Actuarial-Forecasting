...
from methods.model_results import get_model_name
...
def build_model(hp):
    ...
    for i in range(LSTM_layers):
        if i == LSTM_layers - 1:
            model.add(LSTM(hp.Int(f"LSTM_{i}_units", min_value = 32, max_value = 192, step=32), batch_input_shape = (BATCH_SIZE, data['train_X'].shape[1], data['train_X'].shape[2])))
        else:
            model.add(LSTM(hp.Int(f"LSTM_{i}_units", min_value = 32, max_value = 192, step=32), batch_input_shape = (BATCH_SIZE, data['train_X'].shape[1], data['train_X'].shape[2]), return_sequences=True))
    ...
    for i in range(Dense_layers):
        model.add(Dense(hp.Int(f"Dense_{i}_units", min_value = 32, max_value = 192, step=32), batch_input_shape = (BATCH_SIZE, data['train_X'].shape[1], data['train_X'].shape[2])))
        model.add(Dropout(Dropout_prob))
        ...
        for variable in TARGET_VARIABLES:
            for number_of_pca in NUMBER_OF_PCAS:
                for output_steps in OUTPUT_STEPS:
                    ...
                    NAME = get_model_name(end_year, variable, FREQUENCY, output_steps, look_back_year, REMOVE_OUTLIER, VAL_YEARS, number_of_pca)
                    ...
                    NN_results = get_NN_results(best_model, data, VAL_STEPS, TEST_STEPS, look_back_steps, dataset,  BATCH_SIZE, EPOCHS, executions = 3)
                    ...
                    info = {'val_loss': NN_results['val_loss'],
                            ...
                            'period': REVERSE_PERIOD_MAP[end_year],
